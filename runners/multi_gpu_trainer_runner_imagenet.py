...
class MultiGPUTrainerRunnerImageNet:
    def __init__(
        self, 
        data_config,
        diffusion_config,
        ...
        checkpoint = None
        self.ckpt_total_report = None
        self.ckpt_training_report = None 
        self.start_epoch = 0
        self.start_batch_id = 0
        if self.train_config.checkpoint != None:
            checkpoint = self.load_checkpoint(self.train_config.checkpoint)
            self.ckpt_total_report = checkpoint["total_report"]
            self.start_epoch = checkpoint["epoch"]
            self.start_batch_id = checkpoint["batch_id"]
            self.ckpt_training_report = checkpoint["training_report"]
    ...
    def save_checkpoint(
        ...
        # print(self.local_rank, type(self.local_rank))
        # print(f"{saving_path}/{self.local_rank}")
        # saving_path_thread = os.path.join(saving_path, self.local_rank)
        saving_path_thread = f"{saving_path}/{self.local_rank}"
        ...
        checkpoint_dict["batch_id"] = batch_id
        ...
        checkpoint_dict["total_report"] = total_report
        checkpoint_dict["training_report"] = training_report
        ...
        torch.save(
            checkpoint_dict,
            f"{saving_path_thread}/checkpoint.pt"
        )
    ...
    def load_checkpoint(
        ...
        loading_path_thread = f"{loading_path}/{self.local_rank}/checkpoint.pt"
        print(loading_path_thread)
        checkpoint_dict = torch.load(loading_path_thread)

        self.diffusion_config = checkpoint_dict["diffusion_config"]
        self.data_config = checkpoint_dict["data_config"]
        self.unet_config = checkpoint_dict["unet_config"]
        self.classifier_config = checkpoint_dict["classifier_config"]
        self.train_config = checkpoint_dict["train_config"]

        # self.classifier.load_state_dict(checkpoint_dict["classifier"], map_location = self.train_config)
    ...
    def instance_adversary(
        ...
        for itr in range(self.train_config.adversary.instance_adversary_iterations):
            ...
            logits_denoised = self.classify(X_0_t)
    ...
    def train_per_epoch(
        ...
        if self.ckpt_training_report is not None:
            training_report = self.ckpt_training_report
        ...
        for batch_id, (batch) in enumerate(self.train_data_loader):
            if (batch_id < self.start_batch_id): 
                if self.local_rank == 0:
                    t_train_data_loader.update(1)
                continue
            elif wait_for_other_process:
            ...
            if (batch_id + 1) % self.train_config.log.inner_epoch_freq == 0:
                file_name = f"train"
                ...
                print(f"[device:{self.local_rank}] -> checkpoint is saved. epoch={epoch}, batch_id={batch_id}")
    ...
    def train(self):
        ...
        report = pd.DataFrame()
        if self.ckpt_total_report is not None:
            report = self.ckpt_total_report

        for epoch in range(self.start_epoch, self.start_epoch + self.train_config.optimizer.num_epochs):
